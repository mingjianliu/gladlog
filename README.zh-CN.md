...
- **诚实性是硬指标** —— 从 parser 差分预言机到 prompt 覆盖门到 UI 数据忠实性测试,整条链路有确定性验证(见 [可验证性路线图](docs/verifiability-roadmap.zh-CN.md))。
...
2. 游戏内开启战斗记录。同时开启**高级战斗日志** —— 否则没有坐标和 HP 采样,回放与部分分析将无法工作;
3. 打一场竞技场,战报自动出现;历史日志用「导入历史日志…」一次性回灌。

AI 分析需要在「设置」里选一个后端:填 API key(Anthropic 或 DeepSeek),或用你本机已有的 CLI(Claude Code / agy / Codex)—— 后者不用 key。哪个都不配也能用全部本地功能。

详细说明见 **[用户手册](docs/user-guide.zh-CN.md)**;新用户常见疑问见 **[FAQ](docs/FAQ.zh-CN.md)**。
...
架构、纪律与工作流见 **[开发者指南](docs/developer-guide.zh-CN.md)**。
