...
- **`context/`**(10 个文件)—— 顶层拼装器。`buildMatchContext.ts` 拉进几乎每一个 `utils/*` 模块和每一张相关的 `data/*` 表,渲染出面向 AI 的 prompt 字符串。`matchTimeline.ts` / `matchTimelineSections.ts` 构建 `[STATE]`/`[DMG SPIKE]`/`[CD]` 渲染时间线行 —— 这是门规谓词共享规则里「渲染值」的那一端(见下文)。`criticalWindows.ts` 决定哪些秒数要更密的采样。`matchNarrative.ts` 构建叙事文本;`resourceSnapshot.ts` 构建资源(法力/怒气等)快照;`timelineHelpers.ts` 装跨文件共用的工具函数。
