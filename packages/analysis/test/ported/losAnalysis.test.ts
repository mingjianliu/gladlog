...
describe("losAnalysis — position interpolation", () => {
  ...
  it("returns null when timestamp is before first action", () => {
    const unit = makeUnit("u1", {
      advancedActions: [makeAdvancedAction(2000, 10, 10)],
    });
    expect(getUnitPositionAtTime(unit as any, 1000)).toBeNull();
  });
  ...
  it("returns last position when timestamp is after last action", () => {
    const unit = makeUnit("u1", {
      advancedActions: [makeAdvancedAction(1000, 10, 10)],
    });
  ...
  it("interpolates position between two actions (B65)", () => {
    const unit = makeUnit("u1", {
      advancedActions: [
        makeAdvancedAction(1000, 0, 0),
        makeAdvancedAction(2000, 10, 20),
      ],
    });
  ...
  describe("maxGapMs (gap-aware interpolation)", () => {
    it("returns null when the bracketing snapshots are further apart than maxGapMs", () => {
      const unit = makeUnit("u1", {
        advancedActions: [
          makeAdvancedAction(1000, 0, 0),
          makeAdvancedAction(31_000, 10, 20),
        ],
      });
    ...
    it("still interpolates when the gap is within maxGapMs", () => {
      ...
      expect(getUnitPositionAtTime(unit as any, 1500, 10_000)).toEqual({
        x: 5,
        y: 10,
      });
    });
    ...
    it("returns null after the last snapshot once maxGapMs has elapsed", () => {
      ...
      expect(getUnitPositionAtTime(unit as any, 5_000, 10_000)).toEqual({
        x: 10,
        y: 10,
      });
    });
    ...
    it("preserves legacy behavior when maxGapMs is omitted", () => {
      const unit = makeUnit("u1", {
        advancedActions: [
          makeAdvancedAction(1000, 0, 0),
          makeAdvancedAction(61_000, 10, 20),
        ],
      });
      expect(getUnitPositionAtTime(unit as any, 31_000)).toEqual({
        x: 5,
        y: 10,
      });
    });
...
describe("losAnalysis — geometry logic", () => {
  ...
  it("detects when one point is inside a polygon", () => {
    // Blade's Edge central spine x[2755..2804] y[5982..6011] — a plain
    // (non-walkable) obstacle. Lordaeron's tomb used to be the example; it is
    // `elevated` since GH #83, so a point inside it stands on top.
    const inside = { x: 2780, y: 5995 };
    const outside = { x: 2780, y: 5970 };
    expect(hasLineOfSight("1672", inside, outside)).toBe(false);
  });
});
...
describe("losAnalysis — distanceToNearestObstacleEdge", () => {
  ...
  it("returns 0 when inside an obstacle", () => {
    expect(
      distanceToNearestObstacleEdge("1505", { x: -2044.5, y: 6623.5 }),
    ).toBe(0);
  });
...
describe("losAnalysis — segmentIntersectsPolygon corner cases", () => {
  it("covers segment completely inside polygon (line 108)", () => {
    // Blade's Edge central spine x[2755..2804] y[5982..6011], a plain
    // obstacle (Lordaeron's tomb is walkable since GH #83 — two players on it
    // see each other). 40 yd apart, both inside.
    const p1 = { x: 2760, y: 5995 };
    const p2 = { x: 2800, y: 5995 };
    expect(hasLineOfSight("1672", p1, p2)).toBe(false);
  });
});
...
// GH #83 (2026-09-23): geometry fixed against the game's own verdicts
// (losGroundTruthScan.ts — successful heals = not blocked, 2v2 "not in line
// of sight" failures = blocked).
describe("losAnalysis — Ruins of Lordaeron tomb is walkable, height unknown (elevated)", () => {
  it("a line across the tomb with an end inside its outline is unknown", () => {
    expect(
      hasLineOfSight("572", { x: 1285, y: 1665 }, { x: 1285, y: 1650 }),
    ).toBeNull();
    expect(
      hasLineOfSight("572", { x: 1279, y: 1665 }, { x: 1292, y: 1665 }),
    ).toBeNull();
  });
  it("two players on the ground on opposite sides are still blocked", () => {
    expect(
      hasLineOfSight("572", { x: 1285, y: 1650 }, { x: 1285, y: 1680 }),
    ).toBe(false);
  });
  it("another obstacle still blocks outright", () => {
    // from inside the tomb outline, through the small west pillar x[1314..1317] y[1675..1679]
    expect(
      hasLineOfSight("572", { x: 1293, y: 1670 }, { x: 1325, y: 1682 }),
    ).toBe(false);
  });
});

describe("losAnalysis — Dalaran Sewers pillars are diamonds", () => {
  it("a line across the old square's corner is clear", () => {
    // the old square x[1305..1312] y[771..778]; its NE corner (1312,771) is outside the diamond
    expect(
      hasLineOfSight("617", { x: 1300, y: 766 }, { x: 1316, y: 774 }),
    ).toBe(true);
  });
  it("a line through the diamond centre is blocked", () => {
    expect(
      hasLineOfSight("617", { x: 1298, y: 775 }, { x: 1319, y: 775 }),
    ).toBe(false);
  });
});
