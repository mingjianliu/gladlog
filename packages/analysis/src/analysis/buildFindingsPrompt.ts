...
const CHAIN_LEGENDS: Record<string, string> = {
  ...
  "kick-eaten": `- "kick-eaten": your hardcast (facts.interrupted) was interrupted by facts.source's facts.kick, locking the school for facts.lockout seconds (when known: at cast start, nearest kicker was facts.nearestKickerDistYd yd away, facts.kickersInRange in kick range; facts.kickDepthPct% into the cast, where depth <30% indicates a very early kick / juke opportunity). Coach positioning and spacing to cast outside enemy kick range, rather than relying solely on fake-casting. facts.postKick says what the player did in the 5 s after the kick: "pressed Nx but rejected (…)" means they kept pressing and the game refused those casts (the locked school, line of sight, CC) — never describe that as doing nothing, freezing up or waiting out the lockout.`,
