...
const NEW_CANDIDATE_LEGENDS: Record<string, string> = {
  ...
  "kick-priority-missed": `- "kick-priority-missed": at facts.t the enemy healer facts.healer hardcast facts.heal (facts.castS s) on facts.target — the player's kill target, sitting at facts.targetHpPct% HP inside an active kill window — and it landed for facts.healK k. The player's interrupt facts.kick was off cooldown, the player was not locked out and was in range (facts.distanceYd yd), and nobody kicked it (facts.othersFeasible names teammates who could also have; "none" otherwise). Corpus reference (n=facts.refNCompleted completed / facts.refNInterrupted kicked casts in this state): the kill target died within 10 s facts.refDeathInterrupted% of the time when the heal was kicked vs facts.refDeathCompleted% when it completed. This is the heal that mattered — coach kick PRIORITY (save the kick for the heal on your kill target), cite the contrast as descriptive, never claim this specific kick would have killed them.`,
  "kick-priority-team": `- "kick-priority-team": at facts.t the enemy healer facts.healer hardcast facts.heal (facts.castS s) on facts.target — the kill target at facts.targetHpPct% HP inside a kill window — and it landed for facts.healK k. The player could NOT kick it (facts.ownerWhy), but a teammate could — facts.teammates lists who, with their interrupt and distance at cast start — and nobody did. Same corpus reference as kick-priority-missed (facts.refDeathInterrupted% vs facts.refDeathCompleted%). Phrase ONLY as communication ("call the kick", "ask X to save the kick for that heal"), never as something the player should have pressed.`,
  "backlash-dispel": `- "backlash-dispel": at facts.t the player dispelled facts.debuff (facts.stacks stack(s)) off facts.target while the target sat at facts.targetHpPct% HP. Removing this debuff punishes the DISPELLER (facts.backlash; landed: facts.backlashLanded): the dispeller took facts.selfDmgAfterK k damage in the 4 s after (facts.selfDmgBeforeK k in the 4 s before) and their healing output went facts.healBeforeK k → facts.healAfterK k over the same 4 s; the same cast also removed: facts.coRemoved. When facts.cdCcSpell is present, facts.cdCcSpell then landed on facts.cdCcTarget at facts.cdCcT for facts.cdCcDurationS s (DR facts.cdCcDr) with every cleanse still on cooldown. Corpus reference (n=facts.refN dispels of this debuff in this HP stratum): removing it saved ≈facts.refRemovedK k of the caster's damage over 15 s, cost ≈facts.refHealLostK k of the dispeller's healing and ≈facts.refBacklashDmgK k of damage taken in 4 s, and added ≈facts.refCcExposureS s of hard CC on the team that nobody could dispel inside the cooldown. Coach the tradeoff — at this HP the DoT was the cheaper option, and the 8 s dispel cooldown is the part players forget. Cite the numbers as a contrast, never claim the dispel caused a death.`,
