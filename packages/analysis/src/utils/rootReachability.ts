...
export { ROOT_SPELL_IDS } from "../data/rootSpells";

/**
 * The unit's root intervals — the one "could this unit move" root predicate;
 * [ROOT] reachability, the kick run budget (candidates/kickPriority.ts) and
 * the crisis DPS "rooted" gate (crisisDecisionPoints.ts) read it. A
 * root-DR-class aura always counts, whoever the source (a reflected
 * Entangling Roots names the druid as source and target — codex astra
 * review); a root aura outside the DR class counts only when another unit
 * applied it — a guard against self-roots; it also drops a REFLECTED no-DR
 * root (Ice Nova on its own mage), a known exclusion.
 */
