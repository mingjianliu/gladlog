...
/** Item IDs whose on-use is Gladiator's Medallion (ItemEffect.SpellID 336126),
 * generated by the same script. Reliability round 3 W1j: every trinket that was
 * not Adaptation / Relentless used to count as a Medallion, so a player with
 * two non-PvP trinkets was "PvP Trinket available" (0e0663e6). On a 303-file
 * sample every one of 2,719 Medallion casters held one of these items; 79 of
 * 3,831 players the old rule called "Gladiator" held none and never cast. */
const GLADIATOR_ITEM_IDS = new Set<string>(trinketItemIdsData.gladiatorItemIds);
...
/** "None" = equipment is known and holds no PvP trinket (no Medallion item and
 * no Medallion cast); "Unknown" = no equipment to read. */
export type TrinketType =
  "Gladiator" | "Adaptation" | "Relentless" | "None" | "Unknown";
...
export interface IPlayerCCTrinketSummary {
  ...
  /** Racial presses that lock the trinket (round seconds, lock length).
   * Optional so hand-built fixtures stay valid. */
  racialTrinketLocks?: Array<{ atSeconds: number; lockSeconds: number }>;
...
export function detectTrinketType(unit: ICombatUnit): TrinketType {
  ...
  if (trinketSlots.some((e) => ADAPTATION_ITEM_IDS.has(e.id)))
    return "Adaptation";
  if (
    trinketSlots.some((e) => GLADIATOR_ITEM_IDS.has(e.id)) ||
    (unit.spellCastEvents ?? []).some(
      (e) => e.spellId === GLADIATOR_TRINKET_SPELL_ID,
    )
  )
    return "Gladiator";
  if (trinketSlots.length > 0) return "None";
  return "Unknown";
}
...
function getTrinketCastTimestamps(
  ...
  // Relentless is passive; "None" has no PvP trinket to press
  if (trinketType === "Relentless" || trinketType === "None") return [];
  const spellId =
...
/**
 * Two independent locks apply and the later one wins: the trinket's own
 * cooldown after the player pressed it, and the shared lockout a
 * category-1166 racial imposes (2026-08-12) — press Will of the Forsaken and
 * the medallion is locked for TRINKET_RACIAL_SHARED_LOCKOUT_MS (Will to
 * Survive: WILL_TO_SURVIVE_TRINKET_LOCKOUT_MS), which the ledger used to
 * ignore entirely, so 57 of 2695 "trinket in hand, sat in it" windows across
 * 120 local matches blamed the player for a button the game had taken away.
 * Returning the remaining time keeps `on_cooldown`'s "Ns left" honest for
 * both causes.
 */
/**
 * Seconds until `summary`'s PvP trinket is ready at round second `atS` (0 =
 * ready), or null when the player has no trinket to press (Relentless is
 * passive, "None" has none). THE one readiness predicate: the [CC ON TEAM]
 * trinket note, the [DEATH] line's "(PvP Trinket available)", the healer
 * exposure line and the peel / bookmark target state all read it
 * (reliability round 3 W1j: three of them used to ignore the racial lock and
 * the equipped trinket).
 */
export function pvpTrinketRemainingSecondsAt(
  summary: Pick<
    IPlayerCCTrinketSummary,
    "trinketUseTimes" | "trinketCooldownSeconds"
  > &
    Partial<
      Pick<IPlayerCCTrinketSummary, "trinketType" | "racialTrinketLocks">
    >,
  atS: number,
  /** renderGrid: a press counts from the whole second it prints at (a use at
   * 10.8 s prints as 0:10, so from second 10 it is spent) — the PEEL OPTION /
   * CC BOOKMARK lines, whose gate re-parses the rendered second. */
  opts: { renderGrid?: boolean } = {},
): number | null {
  if (summary.trinketType === "Relentless" || summary.trinketType === "None")
    return null;
  const happened = (t: number) =>
    opts.renderGrid ? Math.floor(t) <= atS : t <= atS;
  let remaining = 0;
  for (const u of summary.trinketUseTimes)
    if (happened(u))
      remaining = Math.max(remaining, u + summary.trinketCooldownSeconds - atS);
  for (const l of summary.racialTrinketLocks ?? [])
    if (happened(l.atSeconds))
      remaining = Math.max(remaining, l.atSeconds + l.lockSeconds - atS);
  return remaining;
}
...
export function analyzePlayerCCAndTrinket(
   ...
   * pvpTrinketRemainingSecondsAt reads them with the trinket casts). */
  const racialLocks: Array<{ ts: number; lockMs: number }> = [];
  for (const e of player.spellCastEvents) {
    ...
    if (id && SHARED_CD_RACIAL_SPELL_IDS.has(id))
      racialLocks.push({
        ts: e.logLine.timestamp,
        lockMs: racialTrinketLockoutMs(id),
      });
  ...
  racialLocks.sort((a, b) => a.ts - b.ts);
  const trinketUseTimesS = trinketCastTimestamps.map(
    (ts) => (ts - matchStartMs) / 1000,
  );
  const racialTrinketLocksS = racialLocks.map((l) => ({
    atSeconds: (l.ts - matchStartMs) / 1000,
    lockSeconds: l.lockMs / 1000,
  }));
  ...
  const ccInstancesUnsorted: Omit<ICCInstance, "drInfo">[] =
    filteredCCWindows.map((w, idx) => {
      ...
      if (trinketBrokenWindowIdx.has(idx)) {
      ...
      } else if (trinketType === "Relentless" || trinketType === "None") {
        // no trinket to press: Relentless is passive, "None" has none
        ...
        const remainingMs =
          (pvpTrinketRemainingSecondsAt(
            {
              trinketType,
              trinketUseTimes: trinketUseTimesS,
              trinketCooldownSeconds,
              racialTrinketLocks: racialTrinketLocksS,
            },
            (w.applyMs - matchStartMs) / 1000,
          ) ?? 0) * 1000;
        if (remainingMs <= 0) {
  ...
  const getPlayerCompletedMedian = (spellId: string): number | null => {
    ...
    for (let i = 0; i < starts.length; i++) {
      ...
      while (
        nextStartIdx < allStartMs.length &&
        allStartMs[nextStartIdx] <= sMs
      )
        nextStartIdx++;
...
export function formatCCTrinketForContext(
  ...
  for (const s of summaries) {
    ...
    const ccBreakdown = [...countBySpell.entries()]
      ...
      .join(", ");
    const trinketCdStr =
      s.trinketType === "Relentless" ||
      s.trinketType === "Unknown" ||
      s.trinketType === "None"
        ? s.trinketType === "None"
          ? "no PvP trinket"
          : s.trinketType
