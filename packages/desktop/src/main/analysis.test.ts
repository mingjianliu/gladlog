...
describe("分槽落盘(多模型对比)", () => {
  ...
  it("分槽:换 backendOverride(同后端换模型)重分析不覆盖旧槽,getState 列两槽", async () => {
    ...
    const oldSlot = await s.getCached("m1", "anthropic:claude-opus-5-5");
    expect(oldSlot).not.toBeNull();
    expect(oldSlot!.findings[0]!.title).toBe("Death(claude-opus-5-5)");
  ...
  it("旧 v1 文件读取:getCached 照常返回结果(懒迁移),再分析后升 v2", async () => {
    ...
    expect(Object.keys(raw.slots)).toEqual(["anthropic:claude-opus-5-5"]);
    expect(await s.getCached("m1")).not.toBeNull();
  });
  ...
  it("旧 v1 文件 + backendOverride 重分析:v1 内容归属 settings 默认槽,不被 override 槽覆盖(legacySlotKey 修复)", async () => {
    ...
    expect(
      raw.slots["anthropic:claude-opus-5-5"].result.findings[0].title,
    ...
    expect(
      (await s.getCached("m1", "anthropic:claude-opus-5-5"))!.findings[0]!
        .title,
    ).toBe("v1旧分析");
  });
  ...
  it("deepen 写进 lastSlotKey 槽,不碰其他槽", async () => {
    ...
    const other = await s.getCached("m1", "anthropic:claude-opus-5-5");
    ...
    expect(other!.findings[0]!.title).toBe("Death(claude-opus-5-5)");
  });
  ...
  // global default "claude-opus-5-5" instead of the override slot's
  ...
  it("槽键 backend 段不是已知 AiBackend(如手改配置/v1 迁移占位符)→ 回退 settings 默认后端并 warn(复核 I-1)", async () => {
    ...
    // settings has no aiBackend/aiModels → defaults to anthropic:claude-opus-5-5
    expect(streamCalls).toEqual([{ model: "claude-opus-5-5" }]);
    warnSpy.mockRestore();
  });
...
describe("analyzeWindow(#16 选段分析)", () => {
  ...
  it("正常链路:LLM → 审计 → ok + 落盘;二次调用命中缓存不再调 client", async () => {
    ...
    expect(
      ...
      )["anthropic:claude-opus-5-5:30-60"].entries[0].text,
    ).toContain("At 40s");
  ...
  it("window-multi-finding Task 2:模型一次返回 2 条(同 findingIndex)→ entries 长度 2,各自 title/text 落盘;二次调用命中缓存返回同 entries(非仅首条)", async () => {
    ...
    const onDisk = JSON.parse(
      readFileSync(join(dir, "m1", "windowAnalysis.zh.json"), "utf-8"),
    )["anthropic:claude-opus-5-5:30-60"];
  ...
  it("#21 item11 复核轮修复(红→绿):force=true 绕开缓存的 audit-empty 命中,重新打模型;不传 force 仍命中缓存", async () => {
    ...
    expect(Object.keys(cache)).toEqual(["anthropic:claude-opus-5-5:30-60"]);
    expect(cache["anthropic:claude-opus-5-5:30-60"]).toMatchObject({
      status: "empty",
    });
  ...
  it("LRU:第 21 个窗口写入后最旧 at 的条目被驱逐,文件恰 20 条", async () => {
    ...
    try {
      ...
      expect(cache["anthropic:claude-opus-5-5:0-30"]).toBeUndefined(); // oldest (the 1st) evicted
      expect(cache["anthropic:claude-opus-5-5:2000-2030"]).toBeDefined(); // newest (the 21st) present
    } finally {
      dateSpy.mockRestore();
    }
  });
  ...
  it("跨窗口并发:两个不同 windowKey 同场并发分析,先完成的一方不被后完成的一方覆盖(lost-update 修复)", async () => {
    ...
    expect(Object.keys(cache).sort()).toEqual([
      "anthropic:claude-opus-5-5:200-230",
      "anthropic:claude-opus-5-5:30-60",
    ]);
  });
  ...
  it("版本戳:旧版本条目(promptVersion 不匹配)判 miss,重新调用 client 并用新版本戳覆盖落盘", async () => {
    ...
    const key = "anthropic:claude-opus-5-5:30-60";
    writeFileSync(
      path,
      JSON.stringify({
        [key]: {
  ...
  it("模型切换:同后端换模型(判据 backend:model 的 model 段)→ miss,新旧两条各自独立命中,互不覆盖", async () => {
    ...
    const r1 = await s.analyzeWindow(input(dir)); // default model claude-opus-5-5
    expect(r1.status).toBe("ok");
  ...
  it("0.1s 精度:相差 0.7s 的两次拖拽(30.1s vs 30.8s)落成两条独立缓存,不互相顶掉", async () => {
    ...
    expect(Object.keys(cache).sort()).toEqual([
      "anthropic:claude-opus-5-5:30.1-60",
      "anthropic:claude-opus-5-5:30.8-60",
    ]);
