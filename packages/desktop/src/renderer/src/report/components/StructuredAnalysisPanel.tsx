...
export function StructuredAnalysisPanel({
  ...
  useEffect(() => {
    ...
    setActiveEventIds([]);
    void (async () => {
      const cached = (await bridge().analysis.getCached(matchId)) as AnalysisResult | null;
      if (!cancelled && cached) {
        setResult(cached);
        setState("done");
      }
  ...
  useEffect(() => {
    ...
    const offError = bridge().analysis.onError((d: { matchId: string; message: string }) => {
      if (d.matchId !== matchId) return;
      setState("error");
      setError(d.message);
    });
  ...
  const input = useMemo(() => {
    try {
      ...
      const richContext = buildMatchContext(legacy, friends, enemies, { useTimelinePrompt: true });
      ...
      {result && (
        <div className="rpt-ai-body">
          ...
          {result.hadNarration === false ? (
            <div>
              <p style={{ color: "var(--mute)", fontSize: "12px", fontStyle: "italic", marginBottom: "12px" }}>
                Showing candidate events deterministically. No narration generated.
              </p>
              <FindingsList findings={[]} onSelect={setActiveEventIds} />
            </div>
          ) : (
            <FindingsList findings={result.findings} onSelect={setActiveEventIds} />
          )}

          <ExportButtons findings={result.findings} heroText={`${result.findings.length} findings`} />
        </div>
      )}
