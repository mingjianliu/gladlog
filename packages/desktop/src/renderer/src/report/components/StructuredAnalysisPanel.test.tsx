...
describe("split 箭头:选用其他模型分析(Task 4)", () => {
  ...
  it("菜单分组列出可用后端×模型 + 当前全局默认标记,不可用后端不出现", async () => {
    ...
    // model (aiModels unset → AI_DEFAULT_MODEL.anthropic = claude-opus-5-5).
    expect(
      within(menu).getByText("Claude API · Claude Opus 5.5 (默认)"),
    ).toBeTruthy();
    // other models of the same backend appear, without the default marker
    expect(within(menu).getByText("Claude API · Claude Opus 5")).toBeTruthy();
