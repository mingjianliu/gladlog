...
> 但很脆(五个 PASS 里三个恰好压线 80%),且发现一个规则 bug:accuracy 的两个
> 「未检出」是 12 条审计集上限吃掉的,判官真实敏感性是 10/10。
> **建议先修那个上限再开 Layer B** —— 见报告 §5/§7。sufficiency 仍是真盲区(3/10,
> 第四次复现),按 BACKLOG 14.2 第二方向交给确定性覆盖门。
>
> 下面是原始任务描述,留档。
