...
> 附带:noise 维 50% FAIL → **90% PASS**,证实它的 FAIL 是 accuracy 方差的投影。
>
> 剩余 50 件也已补齐(全 80 件)。7 维 verdict:4/7 → **5/7**,首次过判分门槛。
> 但当时很脆,且发现一个规则 bug:accuracy 的两个「未检出」是 12 条审计集上限吃掉的。
>
> **上限已修**(`d39b34b`:12 → 20,超限取前 10 + 末 10;校验器 + 文档同步单测)。
> 全 80 件在新规则下再评一轮(`scores-det3`),**最终 4/7 → 5/7 → 6/7,余量充足** ——
> 六个 PASS 里只剩 labelBias 压 80% 线,其余 90–100%。
> **Layer B 现在可以开跑。**
>
> 唯一的 FAIL 是 `sufficiency`(20%,8/10 未检出全部零反应,第五次复现)——
> 纯盲区,别再用 rubric 修,按 `eval-ab.md` 交给 `qualityCheck` 确定性覆盖门。
> 报告 §7ter 另记了一个待你拍板的点:sufficiency 同时是最大泄漏源,把它移出特异性
> 检查会让六维升到 90–100%,但那属于「调门规直到变绿」,我没自行采纳。
>
> 下面是原始任务描述,留档。
...
## 2. ~~等我拍板的两个产品决定~~ ✅ 均已拍板并落地(2026-07-22)

> **决定一 = 推成可用**(附带要求:量化 noise 影响)。`getTrinketStateAtTime` 未观察到
> 使用 → 返回 true,类型收紧为 boolean。全语料实测:[OPPORTUNITY] 1491 → 101
> (「状态未知」支撑的 1424 → 0;真 on-CD 机会 67 → 101,原先被未知条目挤占 top-N);
> [CONTESTED] unknown 151 → 0 全部转 available,on CD 46 不动,守恒零丢失。
> noise 影响:template/exact 重复率一字不动(0.304/0.021),token p50 −0.35%。
> **决定二 = 只加离散主动 CD**。7 条(8 个 aura id,Nature's Swiftness 双 id)进
> spellCategories + HIGH_VALUE_PURGEABLE_BUFFS,id 从 EN 语料反向提取、CN 全量语料
> 按 id 复核、dispelType=Magic 来自 DB2;时长为语料 applied→removed p50。
> MISSED PURGE 2251 → 2571(+320;Innervate 0 条——语料无敌方侧坐满 3s 实例,
> 通路由 purgeWhitelist 测试保证)。三道 Layer A 门全绿。原文留档如下。
