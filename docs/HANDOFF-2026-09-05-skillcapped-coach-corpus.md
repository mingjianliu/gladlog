...
>
> **第五种机制的实际影响已量化(2026-09-07,另一会话补):它对本文档的所有测量都是惰性的。**
> `BRACKET_TYPE_ALLOWLIST` 目前**只有 `2v2` 一个键**(限制到 `cd-hoarded` / `missed-cleanse`),
> 3v3 与单排 `allow` 为 undefined、不过滤。而本机库 400 回合的赛制构成是
...
2. **用 high 匹配当 Value-Gate 目标句**,逐条对照 gladlog 在同类事件上的实际输出措辞。按 CLAUDE.md Value-Gate Rule,任何新信号动工前必须先手写目标结论句。**2026-09-06 两次更正,最终是 21 条(不是 88,也不是我中途报的 41)** —— 88 →(去 10 条表扬/更优解极性,谓词只在失误上发射)78 →(去掉映到不发射的谓词)**21**。
   ...
   **我是在写「手工清单会腐烂」的同一个会话里手搓了一份清单,并且漏了一条。**
   ...
   另有 6 条**边界情况,单独列出不并进 21**:`burst-into-mitigation` 5 —— 已接线但在本库沉默,它是进攻类型而本库以治疗视角为主,**沉默是抽样不是退役**,需换 DPS 语料再判(**2026-09-12 已判,GH #75**:不用换语料,换 owner 就行 —— `candidateDiagnostics --owner dps` 在同一 400 回合的 792 个 DPS 视角回合上触发 11.4%、判别力 −4.1pp,这 5 条可以对着真实输出核了);`missed-kick` 1 —— 它根本不是候选菜单类型,由桌面端从 kick 审计派生(`MistakeRule.source` 有 candidate/kick/dispel 三种),是有效目标句但走另一条链路。注意 high 是模型自评,且 79 条仍标 `needs_frame`;「high」≠「教练与产品建议一致」,逐条要分开标「事件一致 / 建议一致 / 前提齐全 / 产品当前是否发射」四项。
   2b. **✅ 第 2 项已实测(2026-09-06)—— 6 个谓词里 5 个通过,`kick-eaten` 是唯一指错方向的。**
   做法:21 条目标句按谓词分组,把教练的转述与 `type_definitions.json` 里 gladlog 的图例并排读。
   零模型调用、零代码改动。

   > **2026-09-12 更正(GH #71,codex 两轮对抗后重新 scope):** 下面这张表**保留作历史证据,不再扩展**。
   > 把 medium 映射也纳入后池子是 149 句(含 #75 解锁的 burst-into-mitigation 27 句),但 medium 的定义是
   > 「同一事件家族,谓词未必抓得到这一条」,所以不能直接当图例的对照目标;改成逐句先过准入
   > (EXCLUDED / UNKNOWN / NON-TARGET / ADMITTED),只对被准入且与图例冲突的句子写差异包。
   > 第一遍 95 句:ADMITTED 14 / UNKNOWN 61 / EXCLUDED 20,**有准入证人的可执行差异 = 0**,按预注册门槛停止。
   > 「五个在产信号被价值门跳过」的说法已撤回。全文与台账:`docs/coach-corpus-admission-audit-2026-09-12.md`。
...
3. **✅ 已做(2026-09-07)。`missed-sync-window` 的核心主张被专家验证,但它判「值不值得进」的前提一条都没带。**
   ...
   **`healer-lockdown` 簇实际在讲三件别的事**(76 条:失误 33 / 更优解 23 / 表扬 20):
   - **16 条映到 `unsynced-burst`,且全是同一形状**——「治疗没被控你就开了爆发」,
     是簇里最一针见血的一批专家判断。而 `unsynced-burst` 开关 **false**,2026-08-29 以
     「触发率 62–66%(每个进攻冷却)、分段梯度 +0.1、在描述常态」下架(GH #50 (a))。
     **✅ 口径复查已做(2026-09-07),假设证伪,下架站得住。**
     假设过:下架依据按「每个进攻冷却」量,而教练只对大招说,可能被短冷却稀释 ——
     与 kick-eaten 排序键量错东西同形。判据事前定死:短冷却占多数则重跑梯度,
     集中在大招则假设死。实测(`packages/eval/scripts/unsyncedBurstScopeProbe.ts`,400 回合 /
     310 事件):**`<30s` 0 条 · `30–59s` 0 条 · `60–119s` 139(47.6%)· `>=120s` 153(52.4%)**。
     被指控最多的是复仇之怒(120s)72 · 狩猎(90s)35 · 狂野怒火(90s)29 · 元素化身(180s)20 ——
     **100% 本来就在大招上**,正典进攻表里没有短冷却,不存在稀释。

     **这个否定结果给出了比翻案更有用的东西。** 教练反复教它(16 条),梯度确实是平的(+0.1),
     两件事同时为真 —— 这是「认知问题 1(教练关心 ≠ 有判别力)」**两边都带数字的实例**。
     而用户 08-29 的裁定是**降级为上下文事实**(「时间线上的爆发/控场事实照旧,只撤掉指控」):
     语料说**值得提**、梯度说**不值得指控**、上下文事实正好是「提但不指控」。
     **这份语料在独立验证那个既有裁定,不是推翻它。** 不要再拿这 16 条去申请复活。
     顺带的小数据缺口:18/310(5.8%)的施放在 `spellEffectGenerated.json` 里没有官方冷却
     (符文武器强化、化身:月神选民),已排除出分桶而非当作短冷却,未追。
