...
> **⚠ 本节已作废(2026-09-06 晚)。数字不要引用,方法不要重复。** 下面这套「按发射点/规则表/开关表
> 静态分层」得出的 **16–37 条(0.8%–1.8%)**,是同一个错误的第三次复发:用**读代码**去回答「它到底
> 发不发射」。此后 codex 又指出第四种机制(生产者函数保留但装配处不再调用 —— 已独立复核为真:
> `ccLockedEvents`/`wastedTrinketEvents`/`deathUnusedDefensiveEvents` 非定义处调用数为 0),
> 以及第五种(`candidateFindings.ts` 结尾的**赛制白名单**,`BRACKET_TYPE_ALLOWLIST`,GH #18
> 2026-08-30 裁定:命中的赛制只保留指定类型,其余降级为上下文)。每加一种机制,上一轮的数就作废一次。
>
> **正确答案与正确做法见 §五第 2 项**:观测真值 `npx tsx packages/eval/scripts/candidateDiagnostics.ts
> --n 400 --json`,实际在产 **21 条**(另 6 条边界情况单列)。`tools/coach-corpus/common.py` 的名单
> 已于 `9cab0bf8` 改名并写明它只回答「桌面端渲不渲成失误行」,不回答「发不发射」。
>
> **保留本节只为记录这条失败序列**:49 → 47 → 37 → 31,四个独立计数、四种判据、四次都漏掉一类。
> 这正是为什么结论是「跑一遍语料」而不是「再写一份清单」。
...
0. **【新,最高优先】拿 §3.3·⑧ 的反例校 `kick-eaten`。前数字已跑,第一版方案已被自己的数字否掉;现在的头号候选是「`switched` 该不该发射」,不是威胁门。**
   ...
   **加强 ① 仍未做**:把窗口内全部施法列进事实行。

   **✅ 加强 ②(排序)已于同日落地,用户批准。** 原提法是二选一「`switched` 该不该发射」,
   审计给出了比两边都好的第三条路:**严重度轴本来就在量错的东西上排序**,而 `switchWasHardCast`
   落地后有了可以排的诚实事实。旧表 `idle 0 / acted 1 / switched 2` 断言「switched 最不可教」,
   这个断言**只对真读了条的 16/292 成立**,对其余 95% 毫无证据。
   新的 `postKickSeverityRank`(`ccTrinketAnalysis.ts`,产品与探针共用同一导出)只分三档,
   每一档都写得出证据:
   `0` idle(整窗零施法)· `1` **acted 与瞬发切换并列** · `2` 真读了条的 switched。
   并列是**故意**的:按瞬发可能是好的脱离(猫形态+野性冲锋),也可能接近被打死
   (5 秒一个瞬发治疗),本模块分不开就不假装分得开。`switchWasHardCast === null`
   (旧归档无数据)留在中档 —— **没有证据不等于证明没事**。

   **前后数字(同 400 回合,`--rerank` 模式)**:触发回合 163,其中超过 cap 的 50(30.7%,
   只有这些可能变);**集合变化 10(占触发 6.1%、占超 cap 20.0%)**,仅顺序变化 5(3.1%)。
   两类刻意不合并统计:`kickEatenEvents` 切片后**不按时间重排**,所以严重度顺序就是模型
   看到的顺序 —— 顺序有意义,但远不如「某条 kick 直接从菜单消失」重要。
   实例 `141470d0`(8 条 kick):`Chrono Flames`(真读了条、最不该教的那种)被挤出菜单,
   让位给一条瞬发 —— 正是设计意图。

   ⚠ **已知副作用,别当 bug**:`f2cf085f` 里 `acted` 被一条更早的瞬发切换挤掉,
   因为两者同档、由既有的时间 tiebreak 决定。可以争论 `acted`(等锁定过期才施法,
   行为上更接近 idle)是否该更可教,但**两个方向都没有证据**,所以维持平手。
   若将来拿到证据,改的是 `postKickSeverityRank` 一处,不是散落的排序表。

   **仍未做**:`kickEatenEvents` 与同族生产者不一致 —— `burstAnswered.ts` 记录同族惯例是
   「按危险选、按时间发」,kick-eaten 切片后没有重排。本次没动,记在这里。

   **第三项已测,负结果,结案 —— `acted` 不与 `switched` 同构。**
   怀疑的是:`acted` 渲染 `waited out the lockout`,但它是**兜底桶** —— `switched` 要求
   「被锁法术掩码已知」**且**「该次施法掩码已知」**且**不重叠,任一未知都静默落进 `acted`,
   那句话就从观测变成假设。事前预期写在先:掩码走官方 DB2、被打断的又都是常见读条技能,
   **预计未知比例很低**。
   实测(同一 400 回合,`postKickSwitchAudit.ts` 已把这三项做成常驻计数):
   **73 条 `acted` 里,被锁法术掩码未知 0 条(0%)、窗口内施法掩码全未知 0 条(0%)、
   真的同学派 73 条(100%)。** 兜底桶在本语料里一次都没当过兜底桶,`waited out the lockout`
   是真实观测,**无需修改**。
   保留的限定:样本是本机库 400 回合、治疗视角为主(唤魔/戒律/神牧/武僧/德鲁伊)。
   但掩码来自覆盖全法术的官方 DB2,0% 更可能是结构性的而非抽样运气。
...
2c. **`kick-eaten` 补位置事实:可行性已验证(go),但卡在官方射程数据 —— 未动工。**
   用户 2026-09-06 选了「补事实」而不是「只改措辞」。目标结论句不用自己写,
   教练已经写好了:「你站在三个人的打断距离里读条」「往右挪就出了 DK 的踢技距离」。

   **可行性探针**(`packages/eval/scripts/kickRangeProbe.ts`,常驻;400 回合 / 386 次打断):
   - **Q1 位置取得到**:被打断瞬间 386/386 = **100%**;读条开始瞬间 312/386 = **80.8%**。
     两个 312 相等 —— 只要匹配到 `SPELL_CAST_START`,位置就必然拿得到。
     缺的 19.2% 全部来自匹配不到 CAST_START,而那正是**引导法术**(法力茶/苦修/冰霜射线),
     它们本来就不发该事件。**缺口可解释,且会诚实写成 unknown。**
   - **Q2 实测 p99 与游戏内真实射程吻合**:Pummel 7.8(真 5)· Kick 9.1(5)· Rebuke 11.6(5)
     · Skull Bash 14.9(13)· Mind Freeze 17.7(15)· Disrupt 30.8(30)· Wind Shear 33.3(30)
     · Counterspell 42.0(40)· Counter Shot 37.8(40)。**p99 追踪真实射程,p50 完全不行**
     (玩家通常站在射程内很深)。

   **设计判断(已定)**:用**读条开始**那一刻,不用被打断那一刻。后者会把「敌人在你读条中途
   冲进射程」也算成你的错 —— 正是价值门第 3 条要挡的「指责别人不会读心」。代价是引导法术
   那 19.2% 写 unknown,那是诚实的 unknown。

   ⛔ **卡住的一步:官方射程数据。** `genSpellReach.ts` 已经在做正确的 DB2 链路
   (`SpellMisc.RangeIndex → SpellRange.RangeMax`),但它的 id 宇宙只覆盖
   `externalDefensiveSpellIds`,**不含打断技**;`spellIdLists.ts` 里也没有打断技名单。
   所以要:(i) 建打断技的 id 宇宙 —— 按 Curated-List 规则应取 DB2 里带打断效果的法术全集,
   再用语料观测反向查完备性,**不能手搓**;(ii) 扩 `genSpellReach.ts` 发射它们的 reachYards;
   (iii) 用本探针的 p99 分布做验收门(与 GH #62 踢技锁定时长同一套办法)。
   **本次会话跑不了**:wago.tools 在该环境不可达(github 200 / wago 000),需要能连 wago 的机器。

   **没有退而求其次用语料 p99 当射程表**,理由:这个事实要支撑一句指控
   (「你在 N 个敌人的打断射程里读条」),射程差 5 码就改变计数;而 p99 在 n=10–21 的几个踢技上
   基本等于最大值、噪声大,且只覆盖**观测到的**踢技。`genSpellReach.ts` 的注释本身就是先例:
   手工假设「外减都是 40 码」在 15 个里错了 6 个。

   **接手即可动工的下一步**:在能连 wago 的机器上做 (i)(ii),然后跑
   `npx tsx packages/eval/scripts/kickRangeProbe.ts --n 400` 用 p99 验收。

3. **✅ 已做(2026-09-07)。`missed-sync-window` 的核心主张被专家验证,但它判「值不值得进」的前提一条都没带。**
   ⚠ **本项原本写的对照集是错的**:`healer-lockdown` 簇 76 条里映到 `missed-sync-window` 的只有 **1 条**
   (medium,内容还是「旋风术白放了」)。真正的对照集要按语义找,是**全语料映到该谓词的 18 条**,
   散在 8 个簇里,`healer-lockdown` 只占 1。

   **`healer-lockdown` 簇实际在讲三件别的事**(76 条:失误 33 / 更优解 23 / 表扬 20):
   - **16 条映到 `unsynced-burst`,且全是同一形状**——「治疗没被控你就开了爆发」,
     是簇里最一针见血的一批专家判断。而 `unsynced-burst` 开关 **false**,2026-08-30 以
     「触发率 62–66%、梯度 +0.1、在描述常态」下架。两件事可以同时为真(认知问题 1),
     但有个值得查的口径差:**下架依据是「每个进攻冷却」的触发率,而教练只对关键的那几次说** ——
     缺口可能在**选择与严重度**,不在概念。
   - 13 条 unmapped 失误讲 gladlog 没有的维度:**谁**该去控治疗(「猎人可以陷阱控住他」)、
     以及**控多了**(「太多精力花在链控治疗,而队伍需要的是把伤害打到战士身上」)。
     **gladlog 没有「在治疗控制上投入过度」这个概念**,方向与 `missed-sync-window` 相反。

   **对照结果(18 条,3 条 high 全部是教科书式正命中)**:
   > 「敌方治疗被连控住、**没有徽章**,战士的**保命冷却也用完了**,学生却没有开」
   > 「三重恐惧落地、连击点满、圣骑士**脱离晕递减**且满血、法师攥着爆发,晕应该立刻出去」
   > 「治疗被控、**所有人都脱离递减**,团早就该开了」

   核心主张成立。但教练在这三句里引用的**前提**,这条线一条都没带:
   **递减状态 2/3 · 治疗徽章 1/3 · 击杀目标的保命状态 1/3**。
   实际发射的 facts 只有 `t / windowEndT / healer / cc / durationS / readyCds`
   加可选 `enemyMinHpPct` 与语料参照数。

   更宽的 18 条里反复出现两个这条谓词**表达不了**的失败模式:
   **窗口存在 ≠ 窗口值得进**——「死亡骑士已经有痛苦压制,所以牧师不慌、不交徽章,一个冷却都没逼出来」
   (同一视频两次)、以及「在术士够不到的时候把昏迷交了,控制过完伤害都没落上」。
   教练判「值不值得」用的是:治疗的**递减 + 徽章**、击杀目标的**保命 + 可达性**。

   **这四样 gladlog 全都有,只是没接到这条线上**(`trinketStateFact` / `computeIncomingDR` /
   `killAttempts` 的 `defensivePopped` / 位置与 LoS)。共用的窗口结构体 `IEnemyHealerCcWindow`
   只带 `fromSeconds/toSeconds/spellName/spellId/healerName`,一个都没有 —— 而它是
   `missedSyncWindowEvents` 与 `unsyncedBurstEvents` **共享**的,补上去两边都受益。
   **这是接线缺口,不是数据缺口**,比 2c 的踢技射程便宜得多(那个卡在没有官方 DB2 数据)。

   未做:补事实本身。那会改在产信号的输出,按本次会话的规矩应先出真实前后对照再定。
