...
2300 档的 Oracle 上界有 135,却仍拆不出 cell —— 因为 archetype 维度再切一刀后每格都
掉到 `N_floor=30` 以下。**2100 档在治疗维度上反而比现在的生产语料更厚**,尤其 3v3
(生产现在总共只有 17 个可用 3v3 cell)。代价是对照组从"2300+"变成"2100+" ——
这是用户裁定,不是默认值。
...
**4. 单排建不出来,所以没换语料。**2100 档单排每场产 ~18 条记录(2v2 是 2 条),
...
**剩下的一步**:在一台空闲机器上跑单排(`BRACKETS="Rated Solo Shuffle" PER_BRACKET=1200`),
再 `mergeCorpora.ts` 三份合一、比对后替换 —— 命令在 corpus-tools README。
