...
**盲区确实存在,但更窄**:v1/v2 两轮独立评分里都有 2/5 件是 `5→5` —— 死亡行**全部**
删光,判官一分不扣。这是纯敏感性失败(特异性无关),是本条唯一经得起复现的部分。
上面两个方向仍然成立,只是问题规模比原先记的小。

**别再用当前套件下结论**:每维仅 5 对、阈值 0.8,单个用例即可翻转 PASS/FAIL。
2026-07-20 的 rubric 改动后六个维度各动 1 对(±20pp),与判官噪声不可分。
重跑前先把 `--source-count` 提到 ≥10。
