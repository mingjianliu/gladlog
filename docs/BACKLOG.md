...
> - **⚠ 新增待拍板:noise 口径修复的重锚定副作用。** d243f4b 把 `templateDuplicateRatio`
>   补进 noise 三档规则后,判官一致性达成(34/35 引用实测数字),但同一批 prompt 的 noise
>   从均值 3.91 掉到 2.83——该比率在结构化时间轴上天然 30%+(35 场均值 0.315,而 exact
>   只有 0.022),三档区间是按 exact 口径定的。方向二选一:(a) 给 template 比率单独定档;
>   (b) 认可新锚定、把模板密度当真实 noise 信号。动 rubric 需重跑校准。
