...
## 42. ~~burst-into-mitigation 覆盖门槛 30% 是拍脑袋值,待定~~(logged 2026-09-14,GH #96)— **已定 60%(用户 2026-09-24「用60 2v2不管」)**

**结论(2026-09-24)**:用户说不知道怎么拍这个数,改成从数据读。方法:每次爆发和「目标完全没墙」的爆发比目标掉血,看覆盖率到多少时掉血明显变少(数据在 eval-private `reports/gh96-coverage-threshold-2026-09-24/`,S2 每 30 场,双方所有 DPS)。只取满血开打,平均掉血与 95% 区间:单排无墙 19.2、覆盖 30–60% 21.3(与无墙分不开)、>60% 15.2;3v3 无墙 18.2、30–60% 13.4、>60% 6.8;覆盖 <30% 时三个赛制都比无墙**掉得更多**(26–35,反应式开墙)。2v2 任何覆盖率都看不出效果,按裁决不特殊处理。门槛改 0.6,下文为 09-14 的历史记录。
...
- **2026-09-24 来源找到了,补丁退役(GH #106)**:缺的 15 秒是永恒狩猎的**第二级**——这一行是按天赋等级叠的(每级 −15),90 − 2 × 15 = 60;语料里 289 个猎杀单元全是 2 级。现在由 `PER_RANK_COOLDOWN_TALENTS`(utils/talentModifiers.ts)按 COMBATANT_INFO 的等级算,数值不变(2 级仍是 60 秒),1 级 / 没点的人按 75 / 90 秒。同一批还修了:冷却行乘 PvP 系数、专精被动(SpecializationSpells)进修正来源、冰霜之柱 45 秒 / 恶魔暴君 60 秒两条孤儿被动补丁(用户裁「改」)。
...
用户要一份可以慢慢看的裁决清单:`docs/archive/rulings-pending-2026-09-18.md`(A 一句话:#42 覆盖门 / 队友卡回合末 / triage 只单排 / 关 #77 #89 #78 #80 #93 #95;B 定形状:#66 #67 #82 #79 #72 #73 #87 #88 #81;C 立项:#68 #69 #70 #83 #84–#86)。裁完写回对应 issue / BACKLOG 条目。
...
**修复进度(2026-09-23 同日,PROMPT_VERSION 99)**:**F #272 已修**(只有受方光环、缺施法事件的敌方外置也出 `[ENEMY DEF]` 行;605 份归档 +442 行,findings 哈希不变)· **A2 已修**(反噬控制单表 `data/backlashCc.ts`;原硬编码把吸血鬼之触 DoT 34914 本身当控制 → 6,397 条假控制行全删 = `[CC ON TEAM] … [DISPEL BACKLASH CC]` 2,769 + `[CC ON ENEMY] … (0s)` 3,628;真反噬 87204 罪与罚此前看不见,+836 行;标签写出类型 `[DISPEL BACKLASH CC: silence|horror]`;候选 death-setup −12(全是 `ccAtDeath=Vampiric Touch` 的 trinket-early 假指控)+3(罪与罚 healer-locked)、position-mistake +1(假 4 s「控制」曾豁免它),58 场暗牧子集隔离对照逐条复现)· **A6 已修**(`[CC AVOIDED?]` 写出光环提供者 `(own)` / `(from <pid>)`:2,667 行中 own 2,586 / 队友 69 / 未知 12;Grounding 转移归功改为要求本人在图腾寿命内施放,107 次里 7 次原是队友萨满的图腾)。309 份 qualityCheck 硬失败 3 → 2,剩 #216/#253 待裁「渲染秒口径」。**#216/#253 已裁已修(PROMPT_VERSION 100)**:用户 2026-09-23「如果一个东西在1秒内转好了 那么治疗也可能没有gcd 即时有了 也来不及 … 1秒内的不要管 甚至可以放宽到1.5秒 不过先1秒吧」→ `REACTION_WINDOW_S = 1`,「好了没按」类指控走 `cdReadyInTimeAt` / `isReadyInTimeAt`(t − 1 s 已转好且到 t 没按),`cdAvailableAt` 状态与 `[RES]` 台账不变;309 份硬失败 2 → 0;605 份:`[DEATH]` Unused −35 项、DEATHS WITH MISSED OPTIONS −23 行、cd-hoarded −4/−4、external-unused −3、cc-avoidable −1。**挂账**:参照表背后的决策点(`crisisDecisionPoints` 墙/控制就绪、`burstWindowDecisionPoints`、`cdTriggerPrior`)仍用状态口径,切换需重生成参照表(1–2 h);crisis-no-response 因此未吃到反应窗;放宽到 1.5 s 只改一个常量。**A 类五项已修(PROMPT_VERSION 101)**:当前 prompt 上逐项复测五项都还在,修法都是把模型原本要猜的事实渲染出来 —— A1 `[DEATH] | dampening: N%`、A3 敌方 `[KICK] ; back M:SS`(打断效果 id 与施法 id 不同时退到踢断者 kit 条目,Skull Bash/Solar Beam;Shambling Rush 官方无冷却不写)、A4 `[SLACK]` 带 `team min HP`(与 `[CONTESTED]` 共用 `teamMinHpPctOver`)、A5 防御类 loadout `lasts Ns`、A7 STAYED 行最近敌人距离区间。605 份:死亡行 4,266 行全带、踢断 8,741、slack 618、loadout 18,334、stayed 1,780,无法解释的行变化 0,findings 哈希不变,prompt +0.56%。顺带:positioningScan G4 正则原先只认单时间戳,**STAYED 行(区间时间戳)从未被核过**,放宽后多核 143 条、违规 0;该扫描变异灵敏度 62.5% 是旧账(文档说 <100% 退出 1,实际不退),另记。**B 已修(PROMPT_VERSION 102,用户 09-23 裁「先加 prompt 规则再量」)**:eval responder 加 EXCLUSIVITY DISCIPLINE、产品 findings prompt HARD RULES 加同义一条;同 50 场同口径:全称句 32 → 8,被 prompt 真反驳 7 → 1(codex 判 14 → 6,但它把「那段时间一直」读成整局,两臂都由我按「句子所指时段内的真反例」复核;材料在 eval-private runs/2026-09-23-gh103-a-items/b-exclusivity/;限定=各跑一次、规则臂 prompt 同时带 A 类改动、产品侧那条未单独量)。复核顺带发现并已修(PROMPT_VERSION 103):HEALER TRAINED 行判据是「被控 ≥ 半个窗口」却印成 `CC-locked through this`,不足一半(含 1–4 s 晕)又一律印 `no CC on the healer`;605 份:235 行改为 `CC'd Ns of Ms — team must peel`,1,331 行「no CC」里 857 行是假的(改印实际秒数),474 行真无控制不变。**C 已复测(用户裁「先确定性复测」)**:C2 血量引用 1/432 不符(且是解析伪影)≈ 已消失;C1 区间 316 个中 228 逐字、45 自划、43 与印出窗口共享一端但另一端不同(含真改错一两秒的,如 3:05–3:16→3:15);产品 findings 解释禁数字,C 只影响 eval responder 与问教练。**2026-09-24 后续两题(用户「跟 codex 讨论……两个议题全做」)**:① positioningScan CC 距离改按生产方渲染秒 [t, t+1] 核验(`ccDistanceClaimWindowS`),变异检出 61.3% → 95.3%、违规 0 → 0、新增核不了 0(8b568c9b);② 反应窗接进生成器,按 codex「DO-DIFFERENTLY」分三件:[CD PRIOR] 计「可施法且反应就绪」秒并印 `ready from`、DPS 危机 hasTool、爆发窗口可行性改「同一秒」机会(旧判据三个事实来自不同时刻);爆发参照表全量归档重生成(手册里的 1.8 万场清单已过期,已改为 6.3 万场);605 份 slow-defensive-response 150 → 116(代码 +8 全是窗口后段真机会,新表 −42 全归因到专属格子对比度 < 3 pp)。行为参照表只有治疗格子,hasTool 只影响 DPS,不需重生成。**C1 收尾(用户裁「做成 eval 确定性指标」)**:`packages/eval/scripts/responseQuoteScan.ts`(库 `provenance/responseQuoteCheck.ts`)写进 eval-baseline 第 2 步,只跟踪不拦截;首读:5.5 基线改边 43/316(13.6%)、B 规则那轮 65/325(20.0%),各一次,分不清是噪声、A 类新数字还是 B 规则,下次 baseline 再读。**E 已修(2026-09-24,用户裁「照 B 类办法修并量」)**:responder 加 CAUSAL DISCIPLINE,FOCUS DISCIPLINE 不再说「决定胜负的窗口」(无据句里约一半是「两个/三个窗口决定了这局」——是我们自己的措辞喂出来的);同 50 场、同一判定者(agy,codex 额度用完)、同一正则:无据因果句 57 → 0,因果措辞句 100 → 32,「decid*」40 → 5、「saved him」9 → 0,回答长度不变;材料 eval-private runs/2026-09-24-gh103-causal/e-causal/。产品 findings prompt 早已禁因果且有 causalLint,未改。复核中新发现:`[CC AVOIDED?]` 2,667 行里 334 行与同秒同技能的 `[CC ON TEAM]` 落地行矛盾(BoS 142 / SW:D 86 / Tremor 77——这三条路径在代码里就叫 breaks,渲染却一律写 did not land),另开 issue。
未动:A1/A3/A4/A5/A7、B、C、E。
...
## 55. 竞技场视线几何:用游戏自己的视线判决校准(logged 2026-09-24,GH #104,从 #83 拆出,用户裁「先记账」)

**学到的**:成功的治疗 = 游戏判「不挡」;2v2 里只能给队友的治疗报「目标不在视野中」= 游戏判「挡」(7 种客户端语言,排除烟雾弹)。工具 `losGroundTruthScan.ts`。3,166 份归档:假「挡」13 张图 ≤ 0.9 %,坏的是 572 废墟 10.8 % / 617 下水道 3.8 % / 2759 笼子 1.2 %;真挡召回多数图 6–61 %,但主要不是图画错了 —— ≥ 8 码漏判里 41 % 离障碍 ≤ 2 码(失败施法没坐标,取 0.5 s 内最近样本,人每秒跑 7 码),37 % 离任何障碍 ≥ 6 码且 60/79 穿过成功施法常看穿的格子(高度 / 宠物目标 / 过时坐标)。日志没有 z。
**做法**:先量再改,不凭眼画;预注册留出集(召回不降且假「挡」升 ≤ 0.1 pp 才收);2D 判不了就返回「未知」(`elevated`),不猜。
**已上线 881766ac**:572 墓顶 `elevated` 留出集假「挡」10.63 → 2.82 %(真挡 26/33 → 17/33,其余 9 条未知);617 两个菱形 3.53 → 0.94 %(11/29 → 9/29);产品 `LoS blocked` 419 → 269。
**遇到的问题**:废墟拟合规则拟合集完美、留出集丢 14/33 真挡 → 弃;全图「空洞拟合」两轮预注册 41 → 44 % / 37 → 39 %,逐图选择两轮互相翻转 = 噪声,未上线(python 脚本只在会话 scratchpad,未进仓库);2759 笼子没动。
**下一步(未开工)**:高度模型(唯一能解决 60/79 的杠杆)/ 失败施法取失败前的坐标或插值 / 宠物目标排除。**在这之前别再做 2D 重拟合。**

## 56. 可观测后果句(logged 2026-09-24,GH #70,#103 E 类在等它)

**用户裁决 2026-09-24(四题全裁)**:① 饰品 = 尝试内解了我们的控制即算逼出、不看血量(「饰品绝对是用来解控制」);② 减伤/外置 = 尝试内 **且** 前 3 s 承伤 ≥ 20% 最大血量(区分力 58% vs 36%);③ 反事实禁令只管过去式与打包票,「下次做 X,这样 Y」式建议理由不禁、不做 lint;④ 产品只上踢断(读条没落地)与治疗被控期间队友掉血两类后果行,配 `[STATE]` 网格血量门规 + 防说过头守卫,「逼出」等 #69 有菜单锚点再上。#103 E 类(过去式因果断言)随之解锁。

用户 09-19 定方向(写实际发生的结果、与因果归因分开、反事实继续禁),09-24 定口径:同单位直接后果 + 带证据门的「逼出」、40 场 × 2 臂、只出样例和报告不改产品。**2026-09-24 夜跑完**(`packages/eval/scripts/consequenceProbe.ts`,数据 eval-private `runs/2026-09-24-gh70-consequence/`,约 3.7M token):后果句 A 17(6 全对)→ B 41(21 全对、18 部分、2 错);过度延伸是主要失败形态(「没人能接你的血」「只能」);B 从未把 `NOT counted` 说成逼出,但 37 条 FORCED 只用上 3 条(菜单里没有表扬锚点 → #69);**现行产品已经在写反事实**(A 9 条全无据,「这样…就能…」式),正则 lint 只抓 5/17。门槛数据:墙在我方击杀尝试内外 HP ≤ 50 % 34 % vs 24 %、前 3 s 承伤 ≥ 20 % 58 % vs 36 %;尝试内饰品 96 % 是解我方控制。**待裁四项**(见 #70 两条评论):饰品门 / 墙与外置门 / 前瞻式建议理由算不算反事实 / 产品形态。
...
## 59. 敌方爆发窗口看不到 Radiant Glory 的复仇之怒(logged 2026-09-25,GH #115,用户裁「新发现的问题记账」)

**现象**:惩戒骑带 Radiant Glory 时没有复仇之怒按钮,复仇之怒由触发产生,日志里记成 **454351**(8 秒、DB2 里**没有冷却**)。敌方爆发时间线 `reconstructEnemyCDTimeline`(`utils/enemyCDs.ts` 约 143 行)直接扫敌方的 `SPELL_CAST_SUCCESS`,只收 `isEnemyCdWindowSpell` 为真的施法 —— 要求进攻技能冷却 ≥ 30 秒,454351 每次都被拒。结果:敌方爆发窗口、对齐爆发窗口,以及所有读「敌方开了冷却」的下游都看不到它。可靠性会话第 1 轮重跑在 `825ca842` 里看到:敌方惩戒骑 11 次复仇之怒,一次都没进窗口。

**不是 GH #106 的回归**:这条路径读原始施法,不读冷却账本;这个门至少从 9e0e019f(2026-09-17)起就在。己方那边账本保留触发的发动(proc-only,载具行 `[PASSIVE]`)是 GH #106 第 2 步的设计。
