...
⚠ **2100 档的 node 建库在本机跑不动**:连续两次被系统内存保护杀掉(第二次已按 README
带上 `NODE_OPTIONS=--max-old-space-size=4096`)。按仓库的 OOM 纪律没有继续加码重试。
真要落地得换台机器、或降 `PER_BRACKET` 分批、或等本机其它会话退出。上面的数字来自
低内存估计器,不是建库产物。
