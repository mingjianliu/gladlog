...
include the first-line MATCHID: <matchId> header as required. Opus 5.5 subagents (`model: "opus"`, resolves to `claude-opus-5-5`), one agent per item, ≤8 concurrency.
...
One judge per item (Opus 5.5); judge only reads blind/items/item-NN/{prompt.txt,response.txt};
