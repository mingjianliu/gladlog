...
| Bracket            | 场次  | 占比  | 字节    | 占比  | feed 读数 | 占比  |
| ------------------ | ----- | ----- | ------- | ----- | --------- | ----- |
...
**还有第三个量,而且它才是意外的那个**(2026-09-09 补,按同一轮计算):这个
bracket 让上游付出多少 feed 读取。feed 是**每一轮单排返回一条 stub**,而六轮
共用同一个 GCS 对象 —— `dedupeByLogObject` 就是为此存在的。所以收 1,300 个单排
对象要翻的是 **7,800** 条 stub,比对象数暗示的深六倍;而深翻页是按平方计费的
(`50 × P(P+1)/2`,见 DATA-COMPLIANCE.zh-CN.md §3),于是单排以 16.4% 的对象
吃掉了这一轮 **71% 的 feed 读取**。它在三个轴里的两个上同时是最贵的那个。
以后再做容量取舍必须看齐三列,不能只看前两列 —— 一个 bracket 的
**stub/对象膨胀倍数**,才是把不大的对象数变成一次又深又按平方计价的翻页的原因。
