...
**Cross-AI review of M3b / M4 / M5 (2026-09-13; codex astra and agy, both REQUEST_CHANGES).** Every cited line was checked before acting.

| Finding                                                                                                                                          | Source | Verdict                                                                                                                              | Action                                                                                                                                                                                                                   |
| ------------------------------------------------------------------------------------------------------------------------------------------------ | ------ | ------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| A choice node listed with `id2: 0` (or an id selecting no entry) reads as every entry chosen, so a modifier could raise a bound without evidence | codex  | confirmed (repro loadouts)                                                                                                           | `choiceSelectionResolved` in `utils/talents.ts`; `talentModifierOwnershipOf` answers `unknown`; tests                                                                                                                    |
| Survival of the Fittest passes ±0.04 with a zero-width interval at 0.667 that excludes 0.650; 35 % vs 33.3 % can cross a counterfactual boundary | both   | confirmed                                                                                                                            | **Demoted to unvalidated after review.** This is a post-hoc decision, not the predeclared rule; recorded as such in the row                                                                                              |
| Round units from one Solo Shuffle player are not independent                                                                                     | both   | confirmed as a gap                                                                                                                   | Player-cluster bootstrap added as a sensitivity report (separate random stream, declared intervals unchanged). Every promoted row stays inside the band; BoP 14 players 0.814–0.879, Astral Shift 20 players 0.398–0.409 |
| M4 probe ignores the recipient (relevant in D4)                                                                                                  | codex  | confirmed as a gap                                                                                                                   | Casts on another unit excluded; result unchanged (0 points had only such casts): 16/149                                                                                                                                  |
| `buffDuration.ts` still asks the plain predicate for a modifier talent                                                                           | agy    | confirmed, different mechanism: another spec's talent reaches the rank-0 early return and skips later modifiers (it never lengthens) | switched to `talentModifierOwnershipOf`                                                                                                                                                                                  |
| An uncertain negative modifier would leave pctMin > pctMax                                                                                       | agy    | latent (no negative row)                                                                                                             | directional widening; test                                                                                                                                                                                               |
| `ccDuration` ignores ranks                                                                                                                       | agy    | latent (all entries maxRanks 1)                                                                                                      | test fails if a multi-rank CC entry is registered                                                                                                                                                                        |
| M5 enlarged the sample (every 30 → every 5) after seeing run 1                                                                                   | codex  | fair                                                                                                                                 | recorded: an adaptive sampling choice with the numeric rule unchanged, not an untouched confirmatory run                                                                                                                 |
| PW:S missed by the pressable test                                                                                                                | agy    | already a stated limitation                                                                                                          | listed separately in the M4 ruling list                                                                                                                                                                                  |

After the fixes (same S2 every-30 slice): the scan reproduces run 2 exactly; switch off findings `6e3d8b37…` / context `30053910…` (the M5 state, no further change); switch on: burst-into-mitigation 69 new / 383 corrected-fact / 0 removed, unchanged.
...
## M6 review (2026-09-14 → 15)

1,190 scripted-queue talents → 332 in the fact-bearing queues (protection / cc / cooldown / duration / dispel) → 118 name a spell the product tracks (`talentScriptedTouchScan.ts`). Reading those 118:

- **Not scripted after all — two SpellMod encodings no compiler read.** Aura 341 (cooldown of a SpellCategories Category) and aura 218 / 219 (pct / flat SpellMod keyed by SpellLabel). Compiled in d0b35723; cooldown table 677 → 951 rows; corpus leg `cooldownTalentScan.ts` (rule committed first): 8 rows supported, holders recast even faster than the talented value on 6 (safe direction), none contradicted. Burst / sync reference tables rebuilt over the full archive (b8ec1198).
- **Cooldown resets / per-event reductions** (Anger Management, Red Thirst, Voice of Harmony, Preparation, Coldthirst, Light of the Sun, Capacitor Totem-style): the real cooldown is shorter than the ledger's, so "was it ready" claims err towards "not ready" — safe for accusations. Not modelled. `recastGapHistogram.ts` distinguishes the two shapes: a sharp floor means a fixed unknown reduction (The Hunt, Havoc: floor at 60 s with no DB2 / hotfix / PvP-rule source — BACKLOG #45 asks the user), a smear means per-event reduction (Alter Time, Capacitor Totem, Force of Nature).
- **CC duration talents missing from CC_DURATION_TALENT_MODIFIERS:** Tar-Coated Bindings (Binding Shot stun +1 s, 94 % pick — the likely mechanism behind CORPUS_DURATION_PATCHES' Binding Shot 2 → 3 s) and Focused Ire (Chaos Nova / Void Nova primary target +1 s in PvP — the unexplained Chaos Nova FLAG). Corpus validation running.
- **Duration extensions** are almost all per-event (Animosity, Fired Up, Capacitance, Shattered Destiny…) — not expressible as a fixed modifier; the two static ones touch ground totems (Totemic Focus) or a 7 %-pick talent (Eternal Sanctity).
- **Dispel side effects** worth a later check against the missed-cleanse predicate: Righteous Protection (Blessing of Sacrifice removes poison / disease, 46 % pick), Emergency Salve (Turtle / Feign Death remove poison / disease, 14 %).
- **Protection procs** (Guided Prayer, Well-Honed Instincts, Last Resort, Dream Guide): user ruling BACKLOG #43 — count toward the crisis with "procced, nothing else needed" phrasing. **Landed 2026-09-17** (user: 「可以做一下」): `responses.proc` in `crisisDecisionPoints`, keyed on the proc's marker aura (Well-Honed Instincts 382912 — `crisisProcMarkerProbe.ts`: 1,039 self-applied, 985 at ≤ 49 % HP; the triggered Frenzied Regeneration is rarely logged as a cast, so its heal id is subtracted too); Dream Guide 1278914 is a hand-out buff, not a marker (22/153 self-sourced), and Guided Prayer / Last Resort have no observable id — all three stay out. Second pass the same day (user 「好 可以 看看效果」): every sibling id of every automatic protection talent through the same probe; Blood Draw 454871, Nature's Guardian 31616 (heal-shaped), Veteran Vitality 441387 and Golden Val'kyr 393108 join as `proc`, Cauterize 87023 and Cheat Death 45182 form the `cheatDeath` arm; Defy Fate / Purgatory / Last Resort 209261 / Battle-Scarred Veteran / Elixir / Whirling Steel rejected on 0–15 events or the wrong HP shape. The Hunt (BACKLOG #45) landed the same day as `CORPUS_COOLDOWN_PATCHES` 60 s, talents included (user: 「60秒」).
