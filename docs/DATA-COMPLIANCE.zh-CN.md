...
**每 6 小时跑一次、一天 4 次**(本机时间 01:00 / 07:00 / 13:00 / 19:00)。(这个 plist 从未装载,归档器退役后已于 2026-09-24 删除。)
具体数字,以及我们为什么认为可接受:
...
- **对方要花多少**:按公开的 GCS 出口费率约 **$100–200/年**,记在一个志愿者项目账上。
  用户已知情并接受这个数字。以后真要压这笔钱,可动的杠杆是**覆盖面**,既不是频率也不是
  压缩:我们存的已经就是他们发出来的那份字节,而只要最终仍把同样那批对象下下来,
  降低频率一个字节都省不下 —— 只有当它导致我们**漏掉**场次时才省。
  (这一条在 2026-09-09 之前写的是「频率」,那是错的。)
...
| Firestore 文档读(feed 查询) | 约 200 万–4600 万次 | $0.06/10 万  | **$1.5–27**   |
| API 侧计算与出站            | 约 1,400 次请求  | —            | < $0.5        |

**合计在 $7 到 $35 之间。** 这个区间的宽度几乎全部来自 Firestore 那一行,原因见下 ——
本节早先的版本报的是「约 $7–10」,那是因为读数里**没有把首次全量扫算进去**。
有两件事是这张表本身说不出来的:
...
- **我们的翻页方式恰好是最伤的那种,而且贵在「深」不贵在「勤」。** 每一轮都从
  ...
  | 一轮的形态                          |  页数 |       读数 |   放大 |
  | ----------------------------------- | ----: | ---------: | -----: |
  | 空转(无新场次,只烧 200 已知的尾巴) |     4 |        500 |   2.5× |
  | 隔几天的追赶                        |    84 |    178,500 |  42.5× |
  | 首次全量扫(窗口约 39,000 条 stub)  |   780 | 15,200,000 | 390.5× |

  所以**稀疏的深轮远比频繁的浅轮糟糕** —— 同样的数据,8 轮 ×14 页是 42,000 次读,
  1 轮 ×84 页是 178,500 次,差 4.25 倍。这跟「跑得少=更礼貌」的直觉相反,而且只对
  读取账单成立:它一个字节的出站都不改变。

  上表 Firestore 的上限就是三个 bracket 的首次全量扫(约 4,570 万次读)。
  **实际到底翻了多深我们压不实**:run log 只打印停页原因,**从不打印页数**,而首轮
  那份日志已经没了。所以 4,600 万是上界不是实测 —— 而且哪天真要复活这个采集器,
  **请把每 bracket 每轮的页数打进日志**,这个几乎零成本的改动本可以让这一节是个事实
  而不是一个区间。如果他们的 resolver 并没有把 offset 直接透传给 Firestore,实际还会
  更低。复活它的人第一件事应该是换游标分页 —— 但要清楚游标只消掉这个平方项:
  它既不减少请求数,也不省一个字节的出站。
...
**一个算到底的例子 —— 单独一轮花了对方多少钱。** 2026-09-04 那一轮是最大的一次:
四个日分片、11,872 个对象、墙钟约 12 小时(`run-2026-09-04.log`,以及框住它的
账本文件 mtime)。

| 成本项                  | 用量                          | 金额            |
| ----------------------- | ----------------------------- | --------------- |
| GCS 出站                | 8.90 GiB(实测 786 KB/对象)   | **$1.07**       |
| GCS Class B 操作        | 11,872 次 GET                 | $0.005          |
| feed 读取,offset 计费   | 3,103,450 次                  | $1.86           |
| feed 读取,若走游标      | 25,850 次                     | $0.02           |
| **合计**                |                               | **$1.09–$2.94** |

有两点值得记住。其一,**这一轮 84% 的读取来自单排** —— 517 页里的 322 页 ——
因为 feed 每一轮返回一条 stub 而六轮共用一个对象,于是一个只占 22% 对象的
bracket 要翻六倍深的页,再按平方计费(三个轴的对照表见
[pvp-log-archive.zh-CN.md](pvp-log-archive.zh-CN.md))。其二,**那 12 小时是我们
自己的节流,不是数据量**:11,872 次下载按 `DOWNLOAD_SLEEP_MS = 2000` 光间隔就是
6.6 小时,占全程 55%,而 8.90 GiB 在任何合理带宽下都只要 15–50 分钟。一轮的时长
跟着对象数走,不跟着字节走 —— 这也是为什么它不是该为对方去优化的那个量。
