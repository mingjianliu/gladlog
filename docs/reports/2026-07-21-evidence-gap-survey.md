...
> **状态已在 §6.5 全部更新** —— 下表是普查当时的原始观察,五项**逐项都有结论了**,
> 别再照这张表判断该做什么。摘要:

| 缺口                              | 覆盖                | 结论(详见 §6.5)                                              |
| --------------------------------- | ------------------- | ------------------------------------------------------------ |
| 敌方大冷却 `none tracked`         | **805 / 1245(65%)** | ✅ 已修 `bf17ccf` → 1/1245                                   |
| trinket 状态 `never observed`     | 1094 / 1245(88%)    | ⚠️ 根因已定位,**修法是产品决定,留给人**                      |
| `[MISSED PURGE OPPORTUNITY]` 为空 | 954 / 1245(77%)     | ✅ 修了确定的一半 `2f1954c` → 822 行升到 2251;剩下是策展决定 |
| `[CONTESTED]` 为空                | 1069 / 1245(86%)    | ⚠️ 与 trinket 同根,一并留给人                                |
| `POSITIONING` 段缺失              | 429 / 1245(34%)     | ❌ 不是 bug,是覆盖边界(治疗拉开距离 = 正确)                  |
...
为了降噪);如果里面混着可教的漏驱散,那就是在丢证据。~~本次未展开。~~ **已展开,见 §6.5 ——
8696 个条目里 26% 是我方真的完成了的进攻驱散。**
...
只缺分类标签,补上。**全语料 1245 场实测**:

| 判据                            | 修前 |                                   修后 |
| ------------------------------- | ---: | -------------------------------------: |
| `[MISSED PURGE OPPORTUNITY]` 行 |  822 | **2251**(+893 Freedom、+536 Sacrifice) |
| 含该段的场次                    |  291 |                         **371** / 1245 |
| `[PURGE]`(我方完成的驱散)行     |  141 |                                **382** |
| `[ENEMY PURGE]` 行              |  148 |                                **297** |
| token p50                       | 6553 |                           6563(+0.15%) |

后两行是**连带提升**而非新增:这些驱散本来就折在 `[MINOR DISPELS]` 汇总里,
现在升为一等公民。逐条核过守恒 —— 唯一「消失」的那条 `[ENEMY PURGE]` 是同秒合并
(`stripped Power Infusion` → `stripped Blessing of Freedom, Power Infusion`),
245 条 `[MINOR DISPELS]` 是汇总行重写(`Greater Purge x3` → `x1` + 2 条提升),
28 条 `[YOU]` 是补上了 `[removed: ...]`。**无内容丢失。**

门规全量复跑全绿:CJK 0/1245、死亡 0/0/0、行内冗余 0、HP 0/6322、death-trace
0/3947、几何 0/24881、qualityCheck 0 硬失败。同时加
