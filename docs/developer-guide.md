...
`npm run presubmit` is a superset of the CI `test` workflow's `static` and `unit` jobs — `verify:doc-commands` runs only locally (`eslint .` from repo root + `verify:doc-commands` + `typecheck` + all-workspace tests + `verify:vision` + `electron-vite build`). Do **not** hand-type the old three-piece (`npm test --workspace=packages/desktop && npm run typecheck && npx eslint .`): it skips `verify:vision` and the production build, and the build step is the only local check that catches a renderer value-importing `src/main/*` — dev and vitest both let that through until the packaged app dies. After pushing, watch it go green with `gh run watch <explicit run id> --exit-status`.
...
Known measurement facts: under Sonnet, a single-round accuracy Δ of ≲0.6 was noise (measured by 2026-07-20 test–retest, paired SD ≈ 1.3, n=50); under Opus 5.0 (calibrated 7/7 on 2026-09-15), A/B-derived paired SD was 0.57 for accuracy and 0.92 for focusCalibration (n=40, 2026-09-16); under Opus 5.5 (the judge since 2026-09-23, calibrated 7/7), the formal test–retest (n=50) gives accuracy paired SD 0.65 (MDE 0.18) and focusCalibration 0.40. Batch responder/judge subagents use Opus 5.5 (`claude-opus-5-5`, since 2026-09-23), the same model as the product coach default (`AI_DEFAULT_MODEL`). From 2026-09-12 to 2026-09-22 both were Opus 5.0, and before that Sonnet: noise floors, judge calibrations and baselines are tied to the model that measured them and are not directly comparable across a switch — re-run `/calibrate-judge` and a fresh baseline before comparing across one.
